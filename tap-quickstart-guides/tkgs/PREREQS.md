# Prerequisites for installing Tanzu Application Platform on vSphere with Tanzu
...
* vSphere with Tanzu v7.0 U3a or later
...
* pod security policies must be configured so that Tanzu Application Platform controller pods can run as root. For more information, see the [Kubernetes documentation](https://kubernetes.io/docs/concepts/policy/pod-security-policy/).
  * Run the following to set pod security policies
```
...
* Allocate a wildcard subdomain for your developer’s applications. Example: `*.apps.tapdomain.companydomain.com`
* Allocate a wildcard subdomain for your workshops and content. Example: `*.learning.tapdomain.companydomain.com`
* Allocate a fully Qualified Domain Name (FQDN) the TAP GUI:  `tap-gui.tapdomain.companydomain.com`
* Allocate a fully Qualified Domain Name (FQDN) for the TAP supply chain security tools:  `metadatastore.tapdomain.companydomain.com`
* Allocate a fully Qualified Domain Name (FQDN) for TAP Application Live View: `appliveview.tapdomain.companydomain.com`
...
* `VERSION-NUMBER` is your Tanzu Application Platform version. For example, 1.3.0.
